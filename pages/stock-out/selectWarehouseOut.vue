	...
	<view>
		...
		<view class="content" v-if="show === false">
		  <checkbox-group @change="checkboxChange" style="display: flex; flex-wrap:wrap">
		    <label :class="item.checked ?  'checkbox selectBox' : 'checkbox '" @click="labelBtn(item.id,index)" v-for="(item,index) in labelList" :key="item.id" >
		      <checkbox :value="item.id" :checked="item.checked" v-show="false"/>{{item.name}}
		    </label>
		  </checkbox-group>
		</view>
...
<script>
	export default {
		...
		methods: {
				...
				for (var i = 0; i < bb.length; i++) {
					for (var j = 0; j < supplier.length; j++) {
						if(supplier[j].name === bb[i]) {
							...
							this.temp_dataList.push(supplier[j].name)
						}
					}
				}
				this.allList = supplier
			...
			labelBtn(name,index){
			  this.labelName = name
			  if(this.labelDataList.join(',').indexOf(name) >-1){ // 解决组件视图不能跟着更新的两种办法
				  this.$set(this.labelList[index],"checked",true)
				  this.temp_dataList.push(this.labelList[index].name)
			  } else {
